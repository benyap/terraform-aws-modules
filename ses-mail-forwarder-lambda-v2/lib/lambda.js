...
exports.handler = async (event, context, callback) => {
    ...
    const { bucket = "", objectKeyPrefix = "", fromEmail = "", defaultRecipient = "", prefixMapping: rawPrefixMapping = "{}", forwardMapping: rawForwardMapping = "{}", } = process.env;
    const prefixMapping = JSON.parse(rawPrefixMapping);
    const forwardMapping = JSON.parse(rawForwardMapping);
    //
    // Parse SES event
    //
    ...
    try {
        const result = await s3
            ...
            .promise();
        emailBodyData = (_a = result.Body) === null || _a === void 0 ? void 0 : _a.toString();
    }
    catch (error) {
    ...
    //
    const emailsToForward = recipients
        .map((rawEmail) => {
        ...
        if (typeof mappedPrefix !== "undefined")
            prefix = mappedPrefix;
        console.log({
            level: "info",
    ...
    emailsToForward.map(async (email) => {
        try {
            ...
            const message = `Failed to forward email from ${email.originalRecipient} to [${email.recipients.join(", ")}].`;
            console.log({
                level: "error",
                message,
                error: error,
